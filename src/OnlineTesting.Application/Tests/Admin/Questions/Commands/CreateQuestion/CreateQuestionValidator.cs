    ...
    public CreateQuestionValidator()
    {
        ...
        RuleFor(x => x.Translations)
            ...
            .Must(t => t.Select(x => x.LanguageCode.ToLowerInvariant()).Distinct().Count() == t.Count)
                .WithMessage("Question translation language codes must be unique.")
            ...
            .Must(t => t.Any(x => string.Equals(x.LanguageCode, Languages.Default, StringComparison.OrdinalIgnoreCase)))
                .WithMessage($"Default language '{Languages.Default}' translation is required for question.");

        RuleForEach(x => x.Translations).ChildRules(t =>
        {
            t.RuleFor(x => x.Text).NotEmpty().MaximumLength(2000);
            t.RuleFor(x => x.Explanation).MaximumLength(4000);
        });
